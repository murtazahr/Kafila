...
// Model implementations register themselves in init, so a binary that loads a
// shard has to link them in. The mlxrunner package does the same for the
// unsplit path; a shard node does not import it, and without this the
// registry is empty and every architecture reports as unsupported.
import (
